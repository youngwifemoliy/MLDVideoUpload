//
//  MLDNetLoggerPlugin.swift
//  MLDLibrary_Swift
//
//  Created by mrtanis on 2018/11/30.
//
...
public class MLDNetLoggerPlugin: PluginType {
    fileprivate let loggerId = "MLDMoya_Logger"
    ...
    public init(verbose: Bool = false,
        ...
        self.output = output ?? MLDNetLoggerPlugin.reversedMLDLog
...
private extension MLDNetLoggerPlugin {
    ...
    func logNetworkRequest(_ request: URLRequest?) -> [String] {
        ...
        if let bodyStream = request?.httpBodyStream {
            output += [format(loggerId,
                              ...
                              message: bodyStream.description)]
        }

        if let httpMethod = request?.httpMethod {
            output += [format(loggerId, date: date,
                              identifier: "HTTP Request Method",
                              message: httpMethod)]
        }

        if let body = request?.httpBody,let stringOutput = requestDataFormatter?(body) ?? String(data: body,
                                                                     encoding: .utf8),isVerbose {
            output += [format(loggerId, date: date,
    ...
    func logNetworkResponse(_ response: HTTPURLResponse?, data: Data?, target: TargetType) -> [String] {
        guard let response = response else {
            return ["❌❌❌", format(loggerId,
                                        date: date,
                                        identifier: "Response",
                                        message: "Received empty network response for \(target).")]
        }
...
fileprivate extension MLDNetLoggerPlugin {
    static func reversedMLDLog(_ separator: String,
                              terminator: String,
                              items: Any...) {
        for item in items {
//            MLDLog.debug("\(item)\(separator)\(terminator)")
//            MLDLog(item,
//                  separator: separator,
